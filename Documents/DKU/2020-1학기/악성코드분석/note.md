...
- 운전자의 행위: App Market 에서 차량용 App 을 다운로드 받은 후 사용
...
안드로이드 경우 특정 파일 구조를 가지고 있음
이 파일 구조를 unpack(압축되어있는걸 풀어서)해서 직접 소스레벨까지 볼 수 있다는 것
...
이 기술이 각광받고 있음
왜냐면...
스마트폰 쓰지? (삼성이라면) 스마트폰 시스템 소프트웨어 업데이트 하라고 권고 받았어 만약에!
자동으로, 무선으로 펌웨어가 업데이트, 설치된다는 것

이 OS의 이미지가 어딘가엔 올라가고 이게 설치될 거라는 것임

그렇다면 클라우드에서 다운할 수 있다는건 os의 이미지를 획득할 수 있다는 것
...
공격모델, 공격시나리오
실제로 공격이 어케 되었는지
방어기법은 다음 주차에 진행한대(중요 3가지 파트래)

# 201118

11주차
Smart car 무선 시스템 해킹(+ 위협 모델링)

## 11-1 영상(첫번째영상)

지난주에 배운 스마트카 인포테인먼트 시스템의 Background 공부였고
...
어떤 유저가 어떤 명령을 했구나 확인하면 이 메시지를 텔레메틱스 디바이스에 전달함 -> 텔레메틱스는 문을 열거나 할 수 있는 센서로 메시지(CAN 패킷) 보내는 구조
...
문을 열고, gps 신호를 받아보는 등등
목적달성함으로써 공격자는 운전자에게 위협을 가하고 프라이버시를 침해하는 공격을 할 수 있다.
...
12주차
Smart car 보호기법(module2,3:암호학 개론 및 대표 암호 알고리즘)

module2,3 를 학습할 것

이 수업 듣고 9,10,11주차에서 학습한 내용에
어떻게 하면 더 안전한 시스템을 만들 수 있는지 고민하는데 활용할 것
...
능동적 공격: 공격자가 앨리스와 밥의 통신에 적극적으로 참여하여 공격

- 재생공격(통신했던 내용을 몰래 알고있다가 나중에 통신에 다시 사용하는)

- 서비스 거부공격(dos): 특정한 인프라, 서비스의 가용성 떨어뜨려 사용자가 정상적으로 서비스 이용할 수 없게 만드는

7페이지의 기법들을 구현해내면 수동적, 능동적 공격에 포함된 여러 공격들을 막을 수 있을 것

8페이지

정보보호 3대 요소(지키고자 하는 목적)(CIA)
...
인증성
정상적인, 앨리스와 밥 본인이 맞는지 확인하는 인증이 있어야한다

책임추적성
만약 앨리스가 메시지 보냈을 때 문제가 생겼다면,
언제 어떤 방식으로 메시지 보냈는지 추적할 수 있어야함
ex 시스템 로그 확인, 포렌식 등

부인방지
어떤 행위를 하고 하지 않았다고 발뺌하는 것을 막겠다는 것

⭐
다양한 암호 알고리즘 배울 것
-> 7페이지의 적용대상중 어떤 것에 적용되는지 / 어떤 요소에 매칭시켜 요구사항을 해결할 수 있는지 / 어떤 공격을 막을 수 있는지
생각할 것

10페이지

암호시스템=암호알고리즘 으로 봐도 무방

스트림 암호, 블록 암호, DH, RSA 등의 특징 알아볼 것

암호 알고리즘으로 암호 프로토콜을 만듦
개체인증? 보안 시스템 설계시 사용함
...
데이터 암복호화 위해 비밀값인 키를 사용

대칭키 기반 암호 알고리즘에선 대칭키를 키로 사용

15페이지

module2를 본격적으로 배워볼 것
...
output이 매우 랜덤한 것이 장점
ex. PC기반(디스크의 정기적 활동, 시스템 클럭값, 시스템 id 변동값 등을 입력값으로 이용하기도 함, 키보드 입력 타이밍 패턴, 마우스 움직임)

- PRNG

고정된 입력값을 받아 생성하는 난수

TRNG 보다 많이 사용하고 속도도 빠르지만, TRNG보다 안전성은 살짝 떨어질 수도 있음(output의 랜덤성도 TRNG보다 떨어짐)

18페이지

seed값은 계속 하나로 쓰는게 아니고 블록단위로 변경되기도 하겠지

19페이지

PRNG 쓸 때 최소한 지켜야할 요구사항

- 임의성

생성된 랜덤값이 결정적인 상태로 유추할 수 있으면 안되고 랜덤하게 보여야한다는 것

- 비예측성

패턴 예측할 수 없도록 해야함

생성된 어떤 값에서 seed를 예측할 수 있게하면 안됨

- seed 요구사항

seed가 입력값으로 들어갈 때도 예측 가능하면 안됨

20페이지

오른쪽 그림은 뭔가 랜덤하지만 줄이 보이는 것을 보니 일정 패턴이 존재하는 것을 볼 수 있음
seed 값을 잘 관리하는 것이 중요
이렇게 티가 나더라도 임의성, 비예측성은 보장할 수 있게 만드는 것이 중요

21페이지

TRNG -> 아웃풋을 뽑아내기에 시간이 오래걸림

랜덤소스를 입력받아 짧은 길이의 랜덤값을 뽑아내게함
이를 PRNG의 입력값으로 사용(seed 요구사항을 충족할 수 있음)
이런 하이브리드 시스템 많이 사용
...
짧은 데이터이건 큰 데이터 이건 특정 길이의 해시 결과를 출력함

23페이지

해시함수가 암호학에서 쓰일 때는
암호학적 해시함수!
3가지 필요조건을 만족시켜야함(나중에 자세히 배울 것)

24페이지

일방향 해시함수

어떤 해시알고리즘이 있을 때 입력값으로 인한 출력값을 계산하는 것은 매우 쉽고 빨라야함
그러나 출력값을 보고 입력값을 예측하기는 매우 어려워야한다
...
KDF-> 암호학에서 키를 만들어낼 때 쓰는 툴. 해시함수가 결합되어 사용되기도 함

26페이지

암호툴-> 안전하지 않거나 오래된 암호툴은 사용 지양
권고사항 꼭 살펴볼 것

SHA1,2,3이 있는 이유는 1,2에 문제가 있어 개선한 것이기 때문
...
KDF로 키 생성하는데 이 툴 사용할 땐
파라미터 설정, 랜덤값 등은 필요한 중요한 이슈

암호키-> 데이터 암호화, 무결성 입증해줄 수 있는 데이터 인증 코드 생성시 사용

키가 어떻게 쓰이는지, 키의 명칭 잘 이해해야함

키 중에서도 데이터 암호화 키, 데이터 인증 키 있을 수 있음

대칭키 암호화는 송수신자가 동일한 키를 가짐
공개키 시스템에는 공개키, 비밀키 있을 것
키는 모두 안전하게 지켜져야하는 비밀값! but 시스템마다 명칭 다를 수 있음

30페이지

데이터의 내용이 외부로 유출되는 것을 막기위함
기밀성 보장 위해 암호화
...
앨리스와 밥이 동일한 키를 가지고 있음(대칭적으로 같은 걸 가지고 있음)

- 공개키 암호

(서로 다른 2개의 키가 사용됨)

앨리스가 밥의 공개키로 암호화된 메시지를 전송
이 암호문은 해당 공개키와 매칭되는 비밀키를 가진 밥만 메시지를 복호화 가능
...
앨리스가 보내는 메시지를 능동적 공격자가 위조,변조,삭제,재전송하는것을 막으려면? 메시지 인증으로 막을 수 있을 것
(삭제는 못 막을수도)

메시지 인증 코드 알고리즘 이용해 output 값(MAC)을 뽑아내는데 이때 비밀키를 이용
앨리스가 보내려는 메시지와 MAC값을 함께 보내면 수신자는 같은 비밀키를 가지고 MAC값을 생성해
자신이 받은 MAC값과 비교함(무결성을 보장하기 위한 행위를 할 수 있겠지)
(그럼 중간에 공격자에 의해 변조될 경우 알아챌 수 있겠지)
...
"데이터 무결성 보장하려면 암호화하면 되겠네요" (X) -> 정의를 잘못 알고 있는 것!

33페이지

암호 알고리즘들은
공개키 기반(Asymmetric)과 대칭키 기반으로 나뉘어짐
...
키 교환
=> Diffie Hellman 기법(공개키 암호화 기법인데 키 교환에도 쓰임)

- 대칭키 기반

블록 암호화, modes of operation, MAC 인증, stream ciphers

35페이지

## 암호 프로토콜

ex. 신용카드를 이용한 온라인 결제시스템에서
카드 정보 입력, 쇼핑몰에 전달해 안전하게 상호 인증 받고 결제까지 가는 시스템
=> 특정한 시퀀스를 가지고 진행되는 통신임-> 이때 다양한 암호 알고리즘, 암호 툴이 결합되서 사용될 것(암호 프로토콜)

36페이지

암호 프로토콜이 왜 필요한지 살펴보자!

앨리스가 자신을 인증하고 싶음(ex 로그인 위해)
앨리스(클라이언트)가 알고 있는 비밀값을 서버로 전송
서버에는 이미 앨리스가 예전에 저장했던 비밀값이 있음
이를 비교하여 두 값이 같으면 앨리스가 통신을 요청한 것이구나 하고 사용자 인증

=> 안전할까? 안전하지 않을까?(능동적 공격자 관점에서 생각해보기) -> 문제가 있다면 어떻게 해결할 수 있을까?

37페이지

공격자 트루디

수동적 공격자 입장에서 키를 캡쳐했다면?
능동적 공격자 입장에서 이를 재전송 공격에 사용했다면?

-> 서버입장에선 이게 앨리스로부터 온 것인지, 트루디로부터 온 것인지 알 방법이 없음. 신원 보장 불가, 메시지에 대한 인증도 불가
키를 이대로 서버로 전달하는 것은 안전하지 않을 것 같음

38페이지

트루디가 이 내용을 못보게 암호화하면될까?
but 클라이언트, 서버 상의 통신에서 상호 인증이나 여러가지 행위를 할 땐 정해진 패턴이 있대
그 패턴속에서 메시지에 대한 의미가 일정부분 드러나기 때문에(ex. 5번 주고받았네 -> 보통 인증때 그렇게 하던데 등등)
암호화되어있다한들 메시지를 캡처하는 것을 막을 순 없음
의미분석까지 되면 이 또한 재전송 가능
서버는 이를 복호화해서 키를 사용하기 때문에 공격 가능성 여전히 존재

=> 메시지 인증을 했어야되는데 메시지 암호화로 해결하려고 했던 것이 실수!

39페이지

그럼 안전한 사용자 인증을 하려면 어떻게 해야할까?

서버는 사용자가 인증 요청시 암호툴을 이용해 랜덤값을 뽑아내 클라이언트로 줌
->클라이언트는 비밀키를 가지고 암호알고리즘을 돌려 비밀값을 생성
->이를 서버로 보내면 이걸 풀어서 원래 보냈던 랜덤값과 동일한 값인지 검증

능동적 공격으로
암호화되기 전후 랜덤값을 캡처해 재전송하면 되지 않냐?라는 생각이 들 수 있음
but
이러한 랜덤값 같은 경우는
한번 사용된 랜덤값은 재사용을 금지한다거나(정책적)
timestamp라고 시간값을 이용
사용한 랜덤값을 클라이언트, 서버가 리스트로 가지고 있게 한다거나
counter 이용해 몇번 사용된 랜덤값인지 확인할 수 있게 추가적인 보안조치 함
=> 재사용 어려움(안전한 사용자 인증 할 수 있음)

40페이지

독자적인 암호 알고리즘, 암호 프로토콜 구축은 위험할 수 있다.
-> 내가 만든 암호알고리즘은 나만 알 수 있고 풀 수 있으니 안전하다는 생각은 위험.
-> 안전성이 검증되지 않은 알고리즘 사용하면 안됨. 전세계적으로, 표준기관에서 인정한 안전한 암호 알고리즘 사용해야함

module1에서 배운거 생각
sw,시스템 취약점 항상 내포 -> 위협 -> 위험 으로 연결될 수 있음

42페이지

## 암호 기본 원칙

Kerckhoffs' principle: 암호 시스템 자체는 모든 내용이 공개되어도 키는 안전하게 지켜야한다.
암호 알고리즘, 프로토콜의 안전성은 알고리즘을 숨기는데 있는게 아니라 `키`를 얼마나 안전하게 지키느냐에 있음

알고리즘은 (표준으로) 다 공개되어있음(aes, rsa, diffie hellman, ...)

sufficient key space principle: 키가 암호에서 사용되려면 키의 사이즈도 충분히 커야한다.

44페이지

암호 체계, 시스템 자체가 비밀로 부쳐져 관리되면 안된다는 것(언젠간 다 노출되고 파악됨)
적은 시스템을 알고 있다.

46페이지

암호알고리즘이라는 체계를 관리하는 것보다
작은 키값을 안전하게 관리하는것이 훨씬 효율적

48페이지

암호시스템을 숨기는 데 급급해 데이터까지 다 노출이 되는... 기밀성 보장 실패

49페이지

- 견고성 향상

알고리즘을 변경하는 것보다 키를 변경하는 것이 훨씬 쉽다
(적합한 예시인진 모르겠지만 은행 보안 카드 잃어버렸으면 카드 하나 다시 발급받으면 됨
그러나 이 비밀카드 사용해서 송금, 신원인증받는 알고리즘 자체가 노출됐다면? 은행은 알고리즘 전체 바꿔야할 것 -> 엄청난 비용이 들것)

- 신뢰성 향상

암호시스템을 공개하면 -> 이를 써야하고 관심있는 국가, 기업, 전문가들은 해당 암호시스템의 효율성, 안전성을 검증하고 서로 공유할 수도 있을 것(공개적 검증 가능-> 안전성 공개적 입증 가능)

50페이지

대칭키 사용해 암호문을 만든 상황 가정
암호문을 트루디가 확보했다면
암호문이 만들어질 때 생성됐던 평문에 대해서도 확보했다면?
공격자는 암호문을 생성해내는 키가 무엇인지 찾아볼 수도 있을것

키가 만약 10비트면 그 비트가 표현할 수 있는 모든 경우의 수를 암호알고리즘에 넣어 확인할 수 있을텐데
이 때 키가 2비트라면 4번의 시도만에 암호화에 사용되는 키를 찾을 수 있겠지
그런데 64비트든 128비트든 충분히 안전한 비트라면? 이 모두를 전수조사해야되는데 현존하는 컴퓨팅 파워로는 전수조사할 수 없는 양이 될 것

우리는 최악의 상황을 항상 고려해야함
공격자가 전수조사 공격을 한다고 생각했을 때 충분히 안전한 크기의 키 space를 가지고 있으면 그러한 공격으로부터 안전성을 확보할 수 있겠지!

51페이지

kisa 한국인터넷진흥원
권고하는 키와 길이 이용 안내서 나와있음

52페이지

256비트 정도 쓰면
2030년 이후에도 안전하다는 건데
비트수가 커지다보면 키 사이즈도 커져 알고리즘 돌아갈 때 효율 떨어짐
-> 적당한, 적절한 크기의 키 사용 필요

안전성과 효율성의 tradeoff를 잘 맞춰 개발해야하기 때문에
...
암호화키 = 복호화키
빠른 속도로 암호화 가능->대규모 데이터에 사용

59페이지

블록 암호
input으로 들어오는 데이터가 특정한 블록 사이즈로 구성됨

스트림 암호
비트나 글자 단위로 연속해서 나오는 메시지를 암호화
...
알고리즘에서 p-box, s-box, xor로 구성된 round 돌리면서
평문이 키를 만나 암호문으로 변환되는 과정에 다양한 변형 발생시킴
평문보고 암호문 유추할 수 없게끔, 암호문보고 평문 유추할 수 없게끔
...
비트나 문자, 바이트 단위로 데이터를 암호화하는 기법

주로 휴대폰, SSL 프로토콜에서 사용

62페이지

대칭키 암호기법을 쓰려면
송수신자가 동일한 키를 나누어가져야함, 그리고 이 동일한 키는 송수신자 각자만 알고있어야함

N명의 그룹이 통신하기 위해서는
각 사람은 N-1개의 키를 가지고 있어야 그룹 내 모든 사람과 통신이 가능

-> 대칭키 기반, 키 관리 부담스러움

63페이지

네트워크에 참여하는 엔티티 개수가 늘어날수록 관리해야하는 데이터 사이즈가 얼마나 커지는지 계산해보기(단순 산술 계산)

64페이지

키 길이(사이즈)에 대한 안전성 이슈
...
장점
공개키 암호 알고리즘에 비해 연산이 빠르다

단점
키관리 어려움->대규모 네트워크에 적용하기 힘듦
키가 많아지면 비밀값이 많아지는거니까 그럼 안전도가 떨어질 수 있겠지

66페이지

데이터가 상당히 클 수 있음(책 한권, 문서 여러장 처럼)
블록 단위로 운영되면 블록 사이즈는 제한적일거 아니여
이 때 사용하는 것이 운용환경(modes of operation)
종류는 그림에 나온 것들이 있을 수 있음
...
대칭키에서의 문제가 동일한 키를 어떻게 안전하게 나눠가질지가 문제임
통신 늘어날 수록 비밀키도 늘어날텐데 어떻게 안전하게 관리할 것인가

71페이지

송수신자가 동일한 키를 쓰자녀
앨리스가 암호화한 암호문이 앨리스가 암호화했는지 밥이 했는지 알 수가 없음-> 둘다 같은 키와 같은 암호/복호 알고리즘을 알고있으니까-> 부인방지 제공하지 못함
-> 그래서 공개키 알고리즘 등장
...
여기서 통신개체가 추가된다해도 밥이 가진 키가 늘어나나? no 아님

밥이 자신의 공개키를 공개해놨는데
이 공개된걸 보고 앨리스가 암호화하면
이 암호문을 열 수 있는 건 밥 밖에 없다 라는 것
!!!
(공개키)

공개키 개념의 키사용을 살짝 바꿔서 사용하면 -> 전자서명
but 공개키 암호기법과 전자서명은 키 사용에 있어서 많은 차이가 있어 추후에 학습할 것

74페이지

(속도가 느리니까)긴 데이터보다 짧은 데이터 암호화시 많이 사용
-> 대칭키 공유할 때 쓰면 좋겠지

PRNG TRNG이용해 하이브리드 난수생성기 만든 것 처럼
공개키 + 대칭키 하이브리드 시키면 공개키를 이용해 안전하게 키를 나눠가진 다음에 대칭키 이용해 큰 데이터를 암호화할 수 있겠지
...
메시지 인증과 관련된 공격이 잇음
메시지 인증만 있으면 막아낼 수 있는 공격이 있다는 거지
-> 메시지 인증 코드 사용

80페이지

mac addr(네트워크)
정보보호에선 mac이 메시지 인증 코드

81페이지

일 방향 해시 함수 혹은 암호학적 해시함수 이용
둘의 다른점은
메시지 인증을 위해선 키를 사용
이 키는 검증에서 사용하는 키와 페어를 이용하기 때문에 대부분 대칭키에 기반

이 MAC값은 메시지 위변조나 재전송에 대해 안전하게 방어할 수 있느냐를 제공해줌

메시지 인증 코드, 암호학적 해시함수 배울 때 더 자세히 알아볼 것

# 201203

13주차
Smart car 보호 기법(인증, 인가, 접근제어, 최신 보안 기술, Smart car Digital Forensic)
...
12/15 오후에 일대일 세미나 결정됨

수업 내용 (논문) 중심의 과제래! (과제는 논문중에 나온대) 수업 내용 기반으로 해결할 수 있도록
논문 스스로 읽어보고 파악하는 시간 필요(논문만 잘 알고 있어도 과제에 어려움 없을 것)
...
⭐
사고가 벌어진 이후 분석하기 가장 좋은건 사이버 킬 체인일 것

stride -> 설계 단계에서 개발하려는 SW,시스템의 문제점을 사전에 인식하고 대응하기 위해서 수행
이걸 상세하게 하려면 DFD 그리고 STRIDE 했음
위협이 이렇게 식별되면 DREAD 로 위험도 측정
-> 여기에 맞는 적절한 보안 조치 적용

네트워크 해킹 -> 논문1
인포테인먼트 시스템 해킹 -> 논문2
=> 이 2개 위협 모델링 해봐
(무선 시스템 해킹 부분까지 논문 있음 힘들까봐 안본 것)

12주차 -> 보안목적 보안요구사항 보안조치 수행해야함
적합한 보안조치 수행하려면 대략적인 보안기법을 어떻게 만드는지 알아야함
암호학 개론과 대표적인 암호 알고리즘을 알아봤음

오늘은 스마트카 보호기법에 쓰일 수 있는 인증, 인가, 접근제어, 최신 보안 기술, 스마트카 디지털 포렌식도 알아볼것

스마트카 디지털 포렌식: 자동차도 이제 디지털 포렌식의 대상

자동차 디지털 포렌식 분야 단국대에서 최초 연구했대(최종무, 조성제, 우사무엘 교수님 + 학생들 + 대검찰청)
...
최신 차량 보안 기술(과제 수행시 도움이 될 것)

- 차량 내부 데이터 암호호와 및 인증
- 침입(차량 내부 네트워크에 침입하는 악성코드 탐지 시스템)
- 디지털 포렌식
- 새로운 보안 개념
- 차량 보안 관련 규제 -> 강화되고 있다는 것(국제 규제 강화에 발맞춰)

6페이지

나의 성적을 확인하고 이의제기를 하는 프로세스를 생각해보자

웹정보 홈페이지 -> 아이디 입력 -> 패스워드 -> 로그인 버튼 클릭 -> 성적 확인 가능

성적확인가능한건 학생. 학생에게 부여된 권한인것임
그 사람이 정당한 권한을 가지고 있는지 확인하려면 단국대 구성원인지 확인해야함
아이디(개인을 식별해주는 요소)
비밀번호(그 아이디를 가진 개체가 정말 맞는지 확인해주는 값)

아이디를 입력하는 것 -> 식별
패스워드 입력하고 로그인 눌러서 정상적으로 로그인되는 것 -> 인증 과정
(아이디, 패스워드는 웹정보시스템 서버로 넘어가서 비교후 매칭 되면 인증이 되는 것)
로그인 됨과 동시에 그 사람의 아이디에 속한 권한이 들어오게됨. 웹정보 시스템에서 특정 행위 가능 -> 인가(특정한 건물에 출입하는 것)
ex. 특정 연구실 들어가려면 학생증 찍어야함->허가된 사람만 들어갈 수 있음. 학생증 카드로 인증을 하고 인가받은 사람만 그 방에 들어갈 수 있음

7페이지

로그인이 잘 통과되면 인증에 성공한 것

인증과 동시에 그 인증받은 사람에게 특정 권한이 부여됨(인가)
이걸 가지고 특정 정보자산 이용할 수 있는거지.
...
ex. 아이디 패스워드 넣은건 개체인증

- 메시지 인증

네트워크상에서 발생하는 특정 메시지나
혹은
어디에 저장된 데이터
가 훼손됐는지 안됏는지 확인하기 위한 것

그림
메시지는 암호키가 잇으면
그걸해시에 넣어서 맥값 뽑은 다음에 이 맥값을 보고 메시지가 변조되었는지 아닌지 확인
...
은행의 서비스 이용한다고 할 때
고객이 우사무엘한테 100만원 입금해달라고 했는데
해커가 중간에 메시지 가로채서 1000만원 입금으로 바꾼다면? 고객은 피해 입게 될 것

-

고객이 100만원 우사무엘에게 입금해주세요 햇는데
이걸 해커가 캡처해놨다가
나중에 재생해서 고객으로 위장하는 경우가 있을 수 있음. '100만원 입금해주세요'x999 할 수 있다는거지..

-> 즉 메시지 인증은,
중간에 메시지가 변조됐냐 안됐냐
이 메시지가 정당한 개체가 생성해서 보낸게 맞냐? 정당한 개체가 지금 나에게 보낸게 맞느냐
등을 확인하는 과정

10페이지

함수에 메시지와 비밀키를 넣음
그리고 생성된 mac(message authentication code)값

보통 이 함수에는 암호학적해시함수를 사용함
송신자와 수신자가 나누어 가진 키를 사용(대칭키 기반으로 쓴대)
키를 가진 사람만 맥값을 생성할 수 있고, 또 검증도 가능함
=> 공격자가 내거를 캡처해 내용을 바꾸게 되면
메시지가 변경되는 것이므로
인풋값이 달라지는 것이니까 아웃풋값도 달라져서 mac값이 틀어지게됨(변조된걸 알아챌 수 있음)

메시지와 함께 맥값을 보내는데
이걸 캡처해서 그대로 재생할수도잇지않냐?
일반적으로, 맥값 만들 때 송신메시지뿐만 아니라 타임스탬프도 들어간대
메시지와 시간이 합쳐져 메시지 인증 코드를 만들면 재생도 막을 수 있을 것임

11페이지

개체로서 식별하는 아이디를 가진 사람이 맞다는걸 증명하기 위한 과정

두가지 엔티티가 개체 인증 활동에 참여함 - 주장자, 검증자

- 주장자: 아이디 패스워드 입력하는 사람
- 검증자: 받아서 인증한 다음 내 서비스를 제공하는 사람
...
- 보통은 인식기반 사용
- 요즘 핫한건 생체기반
- 소유기반 - 많이 쓰는데 서버-클라이언트 모델에 적용하기엔 좀 그렇대
...
디비에 저장된 패스워드 불러와서 비교하면됨!
근데 데이터베이스가 뚫린다면?
패스워드, 아이디 고대로 노출
불법적 로그인 가능

정말 많은 개인정보유출사고가 서버에서 관리하는 아이디, 패스워드를 이렇게 고정된 형태로 사용해서 발생한 거래
지금은 특정 이상 사업장에서는 고정된 패스워드 사용 못한대
그리 멀지 않은 과거에 이렇게 썼다는 것

17페이지

암호학적 해시함수의 특징
입력값 x -> h(x) -> y
y로부터 x를 도출하는건 사실상 불가능해야한다.(계산적으론 가능하지만 그 계산 시간이 무지막지하게 커서 공격자가 엄두도 못내야함)

앨리스가 자신의 아이디와 패스워드 보냄
서버에선 앨리스의 아이디를 찾고 패스워드는 패스워드를 해시한 값인 y가 들어가있음
앨리스가 입력한 패스워드를 해시함수에 넣고 이를 서버에 저장된 패스워드 해시값과 비교해서 같은지 확인함

만약 서버가 털렸다면?
공격자가 패스워드에 적힌 해시값을 입력한다고 로그인 할 수 없을 것

but
전수조사 공격의 하나인 사전 공격에 취약할 수 있대
패스워드로 쓰일 것 같은 모든 입력값에 대한 출력값 리스트를 미리 만들어두는 것
해시함수에는 키를 입력하지 않고 불특정 길이의 데이터를 특정 길이의 데이터로 변환해주는 것뿐
(해시 함수를 응용해서 만드는게 아까 봤던 메시지 인증이고)
여기서 말하는 데이터베이스에는 그냥 해시함수 거쳐 넣음
암호학 에서의 특징 때문에 해시함수는 다 공개되어있음
공격자도 알고 있는 것
알파벳 조합 다 만들어놓고 이를 해시함수 적용한 출력값을 다 리스트로 만들어놓음
(사전에 있는 모든 말을 이렇게 만들어놧다 라는 것)
만약 해시값이 유출됐으면 이걸 그 리스트에서 찾아서 매치되는게 있으면 평문 찾을 수 있겠지

무식한 방법이지만 이렇게 뚫릴 때가 많대
...
이미 고정된 솔트를(비밀값은 아니나 서버에 저장되어있는)

아이디를 만들면 솔트를 랜덤하게 매칭을 시켜주고
패스워드와 솔트를 해시한값을 패스워드에 가지고 있음

앨리스가 로그인하며 패스워드 넣으면
그 패스워드와 솔트를 받아 해시값 만들고
그리고 서버에 저장된 패스워드(패스워드와 솔트가 해시된 것)와 비교함

앞에서 봤던거와 뭔 차이냐고?
공격자가 서버에 있는 값을 턴다 한들
기존의 패스워드에 솔트라는 랜덤한 알 수 없는 값이 섞여있어서
사전같은 입력값을 리스트업 했다고 해도 여기에 솔트를 다 더해서 해시함수시켜서 아웃풋 테이블을 만들어야함
딕셔너리 크기만큼 해시 작업을 해야하는 것. 엄청난 시간을 소모하게 만듦 -> 현대의 컴퓨팅 파워로는 어렵대 그래서 더 강한 방법이래

20페이지

소유 기반 개체인증

도서관에 학생증으로 들어가는 것 -> 인증
엇 교수님 그럼 이 과정은 인가 아닌가요?
=>의 답. 도서관 안에 예약된 컨텐츠를 이용하려고 또 학생증 내밀면 그건 인가가 될 수 있겠지

21페이지

최근엔 안면, 음성.

안면, 보행 -> 인식률 중요. 딥러닝이나 ai기반의 인식기법 사용. -> 상당히 많은 양의 데이터 학습해야함
안면, 보행과 관련해 프라이버시 이슈가 거론이 많이 된대
함부로 불특정한 사람들의 안면데이터, 보행데이터 영상찍어 확보하면 안된대
대부분의 선진국이 그렇대
but 그렇지 않은 나라가 있는데 중국이래
중국의 안면, 보행인식률은 타의 추정을 불허한대
중국 국민을 대상으로 국가차원에서 기술을 만들고 있기 때문. 중국의 기술력 무시 못한대
...
지금은 상당히 많은 곳에서 생체기반인증 사용하고 있대

많은 기업에서 컨소시엄에 들어가서 기술을 표준화하고 있대
...
권한 확인
그 방에 들어갈 수 있는가?까지 보는 것
인증받은 사용자가 수행할 수 있는 일인지? 사용할 수 있는 리소스인지? 보는 것

27페이지

인가는 접근 제어라고도 함

학생은 인가되지 않았으니까 성적 관리 페이지에 들어갈 수 없대

로그인 하는 순간
그 개체가 가진 권한인지 확인하고
그 메뉴를 오픈해줄지 안보이게할지 정해지는 거지

28페이지

인가
접근 할 수 있는 사람은 허가해주고, 그렇지 않은 사람은 제한하는

인가 구현방법은 2가지 있음

- 접근 제어 목록
  자원에 접근할 수 있는지 목록만들고 이에 따라 하는 것
...
인가 과정을 접근제어라고 써진 박스에서 거침

30페이지

객체는 리소스

접근은 객체에다 무엇을 할 수 있는지 나타냄
...
접근제어목록과 접근통제표는 유사하기 때문에 거의 하나로 봄

32페이지

주체가 특정한 객체에 접근해 무얼 할 수 있는지 미리 정해놓는

앨리스가 로그인하면 급여정보, 주소정보 쪽은 활성화해주고
급여정보쪽은 읽고 쓰는거 모두 허용, 주소정보는 읽게만 할 것
...
ACL과 비슷하대
(ACL은 객체를 다 열거해놨다면.. 이건)
객체마다 어떤 행위 할 수 있는지 표로 관리한
...
미리 표를 만들어 놓음
주체나 객체의 수가 너무 많으면 접근 제어 정보를 관리하기 힘들겠지!

35페이지

그래서 나온 방법이 강제적 접근 제어(MAC)
군사환경에서 쓰이던 거래
주체에 비밀 어쩌고 할 수 잇는 레벨 만들어 놓고
객체에 그 레벨에 매칭된 민감도 레이블 만들어놓고 접근 규칙 만들어놓음

탑시크릿 레벨이면 어떤 행위할 수 있고
시크릿 레벨이면 어떤 행위할 수 있는지 접근규칙에 있음
접근규칙에 맞춰 민감도 레이블에 매칭되는 특정 행위할 수 있겠지

특정한 주체가 객체에 접근하려는 것을 이런식으로 컨트롤

37페이지

예시

객체 파일은 2개가 있는데 하나는 탑시크릿, 하나는 컨피덴셜이래

그럼 각 계층에서잇는 객체만 이용할 수 있나?(탑은 탑끼리 컨피덴셜은 컨피덴셜끼리)
아니면 시크릿은 상위 레벨이니까 컨피덴셜꺼 볼 수 있나?
-> 이런걸 접근 규칙에서 정함
...
자기보다 높은 레벨에 있는 파일은 열어볼 수 없다

39페이지

- no write down

시크릿 레벨이 실수로 자기가 작성한 파일을 컨피덴셜쪽에 저장하면
노출되지 말아야할 정보가 하위 레벨에 노출될 수 있음
이를 막기 위해 쓰는게 no write down

40페이지

직급이 아니라 역할에 따라 접근제어를 정하는 것

역할에 맞춰 접근할 수 있는 리소스를 할당함

말단 직원이지만 인사팀의 직원은 모든 직원의 연봉할 수 있음. 다른 직원은 알 수 없고
아무리 높은 직급직원이라도 개발팀의 sw엔 접근 할 수 없음. 왜? 실수하면 안되니께
이런식!

특정 역할 정하고 그 역할 수행할 수 있는 사용자를 연결
그리고 역할 과 매칭되는 객체도 매칭시킴
어떤 주체가 새롭게 나타났을 때 역할에만 매칭시켜주면 역할에 매칭되는 리소스를 다 쓸 수 있겠지

42페이지

자동차 내부 네트워크에 오가는 패킷을 모니터링해
제어기를 제어할 수 잇는 패킷을 재생해서
자동차를 강제로 제어가능
->can에 어떤 보안기술도 들어가 있지 않아서 가능한 일이라고 했음

2008부터 내부 네트워크 안전성 보장위한 연구 진행됐음
지금은 많이 보급되서 최신 차량에는 많이 탑재됏대
(HSM)
차에 들어가는 연산 장치는 연산능력이 낮대->제어장치의 펌웨어에 sw 구현해 어떤 암호알고리즘 돌린다? 이 알고리즘 처리하는데 시간 많이 걸림->HSM은 그 암호 알고리즘 처리를 빠르게 가속화시켜주는 것

암호화,인증-> 키가 필요
키를 아무데나 두면 안되자너
키를 안전하게 관리하는 키 저장소도 HSM에 사용 가능

43페이지

키 관리, 키 분배
키 통해 데이터 위한 데이터 암호화/인증도 가능

아래 그림과 같은 프로젝트도 있음
참고논문 1 내용보면 후반부에 방어기법 나오는데 그중 하나가 대칭키 기반의 데이터 암호화와 인증이래
⭐
논문 기반 여러 작업하려면 논문의 후반부 참고하면 좋대(HSM통해 데이터 암호화/인증 하는 것 설명하고 있대. HSM은 데이터 암호화를 도와주는 기술이지 HSM이 없다고 안되는 것은 아니래)
내부네트워크의 can 데이터 암호&인증에 대한 연구는 논문1에서 확인할 수 있고 여기서 더 발전한 기술이 HSM이다~

44페이지

차량안의 악성코드를 식별하는 방법 중 하나-> 자동차 침입탐지 시스템(정말 다양함)
그중 핫하게 떠오르는게 ai나 머신 러닝을 이용한 침입탐지시스템임. 그중에서도 단순히 패킷내용만 보는게 아니라 패킷의 전력 신호(아날로그 신호)로 공격자를 탐지해내는 기술이 많이 진전이 됐대

아반떼 1000대 만들면 1000대에 같은 제어장치가
들어갈텐데 이제어장치들의 전력신호 파형이 조금씩 다르대
같은 모델이라도 이렇게 미세하게 다르니까
이를 확인할 수 있는 오슬로스코프라는 장치를 활용해서
어떤애는 주황색, 어떤애는 파란색 신호처럼 나온데
이를 미리 학습시켜서
엔진 이슈의 파형은 파란색이야 그러면
공격자가 주황색같은 파형일 때 걸러낼 수 있겟지

45페이지

침임탐지와 함께
자동차에서 발생햇던 특정 현상에 대해 분석할 수 있느 ㄴ기술들이 추가적으로 나오고 있대

ex. 이벤트데이터 레코더, 자율주행을 위한 데이터 저장 시스템

이 3가지 덩어리가 자동차 디지털 포렌식에 적극적으로 활용된대
여기서의 IDS는 포렌식 관점에서도 쓰일 수 있음

단국대에서 진행한 연구로 정리한 결과래
EDR, DSSAD 관련 분석은 우사무엘 교수님
AVN 이라고 네비게이션 관련 디지털 포렌식은 최종무 교수님 연구실
차량용 어플리케이션에 대해 포렌식 진행한 조성제 교수님 연구실

자동차안에서 발생하는 디지털 데이터를 범죄 수사 관점에서 어덯게 쓰일 수 있는지 연구하셨대

자동차 내부 데이터를 축적하고 있는게 이 3가지가 있었대(edr, ids, dssad)

ids
자동차 내부에서 의도하지 않은 행위 발생되면 탐지하고 로그 남김
-> 자동차 사고시 운전미숙인지 외부로 부터 들어온 사이버테러인지 확인하는데 아주 중요한 단서 될 것

edr
에어백 터지기 전에 속도가 얼마였고 등등 분석이 된 상태.
짧은 시간 남아있는 데이터가 운전사의 사고 직전 직후 특정 상태를 파악할 수 잇는 데이터로 쓰일 수 있음

dssad
자율주행에서 꼭 필요
지금은 자율주행 인간+sw
자율주행시 중간에 인간이 잡는 경우 -> 이런걸 체크하는게 dssad
그래야 사고났을 때 자율주행시스템이 사고냈는지 사람이 냈는지 판별할 수 있을 것

46페이지

라인 킵 시스템 관련
정말 이 시스템이 안전하게 구동됐었는지 사람이 잘못했는지 구분하기 위함

47페이지

edr
국가별로 룰이 다 다름
(공통) 보고 교통사고 났을 때 직전 직후 데이터 가지고 사고 원인 분석하는
...
네비게이션에서
어떤 os 쓰는지 파일시스템쓰는지 분석
->

50페이지

이들이 가진 데이터 분석햇음-> 전화 기록, 전화번호부 등드 다 남아있다는 것
누가 언제 썻고 등(범죄수사에 유용할 것)

51페이지

최근에는 안드로이드 오토나 애플의 카플레이같이
자동차에서만 쓰는 전용 어플이 있대
이걸 이용 자동차에 연결해보는 것
...
자동차에 연결하고 어플 쓴다음
스마트폰에 내용 검토해봤더니
어떤 자동차에 연결됐었는지 기록이 다 남는대
용의자의 스마트폰 리버싱,포렌식해보면
이 사람이 어떤 차에 탔었는지 알 수 있다라는거지

53페이지

무빙 타겟 디펜스

무빙한다. 보호해야할 대상을. 방어를 위해서!

예전에 운영하던 sw,시스템은 정적인 설정을 이용했기 때문에 공격자가 장시간 분석하면 파악하고 공격가능
but 무빙 타겟 시스템은
아무리 오래 지켜봐도 형상을 알아볼 수 없다!라는 것

54페이지

예전에는 공격자 우위 관점
sw,hw를 공격자가 구매해서 분석-> 지능화 고도화 공격을 방어하는데 한계가 있음

그래서 공격 노출점을 교란 시킴(데이터가 저장되는 위치, ip주소, id같은거 바꿔버리는)
분석하는데 많은 어려움 생길 것
proactive한 기술이 될 수 있다

55페이지
많은 시간을 들이면 많은 정보 얻을 수 있고 높은 확률로 공격에 성공 가능

56페이지

그런데 만약,
서버로 접속하는 ip를 매번 변하는 걸 쓴다면
서버 ip가 계속 변한다면
서버에 접근해 악성행위하는게 어려울 것

58페이지

이 개념을 자동차에 대입한 연구가 있음

자동차는 캔 통신 할 때 특정 id를 가지고 브로드캐스트 한다했지
공격자는 id를 미리 유추해 id뒤에 데이터를 붙여 그 개체인 척하며 악성행위를 했었음

근데
무빙 타겟 디펜스를 넣음으로써
id를 알아보기 힘들게 매번 바꾸는 것(one time id)
이렇게 되면 공격자는 공격에 사용되는 id조차 분석할 수 없게됨
...
사이버 킬 체인 관점에서 봤을 때
분석을 못하고
분석안하고 공격하려고 하더라도 통신에 사용되는 주소가 계속 바뀌게 되기 때문에 무기화에서 만들었던 무기가 공격과정에서 전혀 쓸 수 없게 된다는 것

⭐
논문1 에서 보안기법 쓸 때 무빙타겟디펜스를 생각해보면 좋겠지만 짧은시간 설명해준걸로는 어려울 것. 이거에 대해 억지로 보호기법 설계에 사용하려하지말고 이런 기법이 있다 정도만 알기
53,54잘 살펴만 보기
...
몇년전만 하더라도 자동차 관한 사이버 보안? 신경도 안썼음

but 이제 법적 규제 만들어지고 있음
국내에선 katri라는 단체가 규제 만들고 있대

international 주목

61페이지

unece wp29라는 단체에서 자동차 사이버 보안 법령을 만들고 있대
유럽에서 만드는 건데 얼마나 파급효과가 있습니까? 라고 묻는다면
여기에 1958조약에 따라 가입되어있는 국가가 엄청 많대(우리랑 일본도 가입되어있음)
여기서 만드는 규제를 따르고 있대
가입된 나라에 차를 팔려는 다른 국가도 그 규제를 따라야함
전세계에 통용이 될거라고 전망하고 있음

62페이지

자동차를 단순히 product 차원에서의 보호만 하는게 아니라

체계적인 개발하는 과정
프로덕트차원
백엔드, 이 시스템 운영하는 기업 차원
에서의 보안 모두 고려해야함
...
우리 수업
자동차에서 활동하는 악성코드 알아보고, 분석하고, 방어기법까지 세우는 것

규제-이젠 자동차 쪽에서 보안이 엄청 중요해서 법적으로까지 강제되고 있다는 것

⭐
논문1,2 미리 보고 미리 모델링하면 정말 편하게 과제할 수 있을 것
